...
public class AntlrToMyMethodBody extends exprBaseVisitor<MyMethodBody> {
	...
	public MyMethodBody visitMyMethodBody(MyMethodBodyContext ctx) {
			...
			if (local_methodvar.containsKey(i.varName)) {
				...
				if (checkIfAssignmentTypeMatchesRHS(i, i.expr, this.local_methodvar)) {
					...
					this.semanticErrors.add(
							"Error: variable " + i.varName + " return type does not match expression return type.");
				}
			} else {
		...
		for (IfStatement i : ifstatement) {
			if (getCondType(i.cond).equals("NO")) {
				if (!semanticErrors
						.contains("Error: condition " + i.cond.toString() + " error. LHS and RHS must match")) {
		...
		for (MethodCall v : methodcall) {
			if (v instanceof VoidMethodCall) {
				...
				if (global_methodname.contains(((ReturnMethodCall) v).methodName)) {
					if (!semanticErrors
							.contains("Error: " + ((ReturnMethodCall) v).methodName + " is not a void method call")) {
						semanticErrors
								.add("Error: " + ((ReturnMethodCall) v).methodName + " is not a void method call");
					}
				}
			}
	...
	private boolean checkIfAssignmentTypeMatchesRHS(Assignment a, Expr rhs, HashMap<String, Values> var) {
		...
		if (rhs instanceof VoidMethodCall) {
			...
			if (checkIfMyMethodContainsReturnMethodCall((ReturnMethodCall) rhs, this.global_mymethods)) { // if rhs
																											// methodcall
																											// is
																											// declared
				String rhsMethodName = ((ReturnMethodCall) rhs).methodName;
				for (MyMethods m : this.global_mymethods) { // grab method from DeclaredMethodsList, find matching
															// method, check for return data type against type
					if (m.methodName.equals(rhsMethodName)) {
						...
						if (RHSparams.size() != methodparams.size()) {
							semanticErrors.add("Error: " + ((ReturnMethodCall) rhs).toString()
									+ " must have the same number of parameters as mymethod " + m.methodName);
						} else {
							int i = 0;
							for (Map.Entry<String, String> map : methodparams.entrySet()) {
								if (!(this.local_methodvar.get(RHSparams.get(i)).getType().equals(map.getValue()))) {
									semanticErrors.add("Error: dataType of " + RHSparams.get(i) + " in "
											+ ((ReturnMethodCall) rhs).toString() + " is not the same as dataType of "
											+ map.getKey() + " in mymethod" + m.methodName);
								}
								i++;
							}
						}
						...
						} else { // if m.methodType instanceof MyVoidMethod it would have been detected earlier
									// ignore else case
				...
				this.semanticErrors.add("Return Method Call on RHS is not declared: " + a.varName
						+ " cannot be assigned to: " + rhs.toString());
				return false;
			}
		} else
			return false;
